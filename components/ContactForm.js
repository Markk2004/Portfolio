...
export default function ContactForm({ locale, content }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-card mx-auto max-w-2xl space-y-5 rounded-2xl p-6 md:p-8" noValidate>
      {status && <p role="status" className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 p-3 text-sm text-cyan-200">{statusMessage}</p>}
      {[
        ...
        <div key={name}>
          ...
          <input id={`contact-${name}`} name={name} type={type} value={formData[name]} onChange={handleChange} disabled={isSubmitting} aria-invalid={Boolean(errors[name])} aria-describedby={errors[name] ? `${name}-error` : undefined} className="w-full rounded-xl border border-slate-700 bg-slate-950/60 px-4 py-3 text-white outline-none transition focus:border-cyan-400" />
      ...
      <div>
        ...
        <textarea id="contact-message" name="message" rows="5" value={formData.message} onChange={handleChange} disabled={isSubmitting} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? 'message-error' : undefined} className="w-full resize-y rounded-xl border border-slate-700 bg-slate-950/60 px-4 py-3 text-white outline-none transition focus:border-cyan-400" />
        {errors.message && <p id="message-error" role="alert" className="mt-2 text-sm text-red-300">{errors.message}</p>}
      </div>
      <button type="submit" disabled={isSubmitting} className="w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-6 py-3 font-bold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50">
